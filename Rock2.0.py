...
class RockState(object):
    ...
    def same_state(self, state): # Ammar
        old_state = self.encode_state(state)
        old_state['grid'] = 0
        current_state = self.encode_state(self)
        current_state['grid'] = 0
        return current_state == old_state

    def encode_state(self, state): # Ammar
...
class RockEnv(Env):
    ...
    def __init__(self, board_size=7, num_rocks=8, use_heuristic=False):
        ...
        #self.board_size=board_size #Ammar
        #self.init_game()    #Ammar
        ...
        if action < Action.SAMPLE.value:
            if action == Action.EAST.value:
                if self.state.agent_pos.x + 1 < self.grid.x_size:
                    ...
                    reward = 10
                    self.done = True
                    return ob, reward, self.done, {"state": self.state} # {"state": self._encode_state(self.state)} Ammar
    ...
    def reset(self):
        ...
        self.state.agent_pos = self._agent_pos          #Ammar
    ...
    def _get_init_state(self, should_encode=True):
        ...
        rock_state.grid = self.grid                                 # Ammar
    ...
    def print_state(self, state):
        print(self._encode_state(state))
    ...
    def same_sate(self, state):
        old_state = self._encode_state(state)
        old_state['grid'] = 0
        current_state = self._encode_state(self.state)
        current_state['grid'] = 0
        return current_state == old_state
