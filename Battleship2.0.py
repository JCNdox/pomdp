...
class ShipState(object):
    def __init__(self):
        ...
        self.remaining_actions = None    #Ammar
        self.grid = None                  #Ammar

    def same_state(self, state):         #Ammar
        old_state = [state.remaining_actions, state.total_remaining, [e.pos for e in state.ships]]
        current_state = [self.remaining_actions, self.total_remaining, [e.pos for e in self.ships]]
...
class BattleShipEnv(Env):
    ...
    def step(self, action):
        ...
        if self.state.total_remaining == 0:
            ...
            self.done = True
        self.tot_rw += reward
        self.state.remaining_actions = self._generate_legal()   #AMMAR
        self.state.grid = self.grid      # Ammar
        ...
        if mode == 'human':
            if not hasattr(self, "gui"):
                ...
                for ship in self.state.ships:
                    ...
                    for i in range(ship.length-1):              # Ammar add a -1 otherwise to many black cell
        ...
        for length in reversed(range(2, self.max_len)):
            ...
            while True:  # add one ship of each kind
                ...
                if not self.collision(ship, self.grid, bsstate):
                    break
            #print(ship.pos, ship.length)            # Ammar
            self.mark_ship(ship, self.grid, bsstate)
            bsstate.ships.append(ship)
            bsstate.remaining_actions = self._generate_legal() # Ammar
        bsstate.grid = self.grid   #Ammar
        return bsstate
    ...
    def get_possible_actions(self):
        return self.state.remaining_actions
    def print_state(self, state):
        print(state.remaining_actions, state.total_remaining, [a.pos for a in state.ships])
    def set_state(self, state):
        self.grid = state.grid
        #for ship in state.ships:
        #    self.mark_ship(ship, self.grid, self.state)
        self.state = state
        self.done = False
